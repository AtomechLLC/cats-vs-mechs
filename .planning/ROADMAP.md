...
- [x] 03.1-02-PLAN.md — `[S02]` D-14 answered: `actionApCost` / `actionDamage` / `bestPair` generalise `bestDamage` and `factionDps`; `actionModelled` / `unmodelled` name what the figures leave out; `affordability` is ACT-06's read; the shield reading written down beside `DAMAGE_KEYS` (wave 2)
- [x] 03.1-03-PLAN.md — `[S05]` the three-layer action-id gate, `createAction` / `renameAction` / `removeAction`, `actionsNaming`, and a new `[S09.10]` suite with a row per reserved key per write shape (wave 3)
- [x] 03.1-04-PLAN.md — the page reads the student's rules: `refActions` points at the build slice and the tripwire turns on purpose, action names get their own gate exemption, the strip gains the admission line, the picker gains the line beside Remove (wave 4)
...
**Plans**: 16 plans (05-11 runs last)
**UI hint**: yes
...
- [x] 05-05-PLAN.md — hand rulings: `setFightShield` (the writer named for this plan two phases ago), `nudgeFightHp`, the withheld `setUnitHp` / `setAlive` arms, and a ruling recorded as an event in its round rather than as a flag on a value — `[S09.3]`'s shield tripwire turned (wave 5)
- [x] 05-06-PLAN.md — the shell: two topbar controls spending D-04's last reservation with the closure written down, an in-page fight region at the top of the page, a ledger root OUTSIDE `#board`, `[C14]`'s frame, and `KNOWN_IDS` + stubs arriving with the ids (wave 6)
...
proposal). Three refinements arrived before execution and are recorded in the same file's addendum:
team resources sit above the action picker; targeting defaults to the lowest-health enemy with a
change-target button instead of a picker every time; and the fight tab carries a **battlefield
visual** of the current round — new work, and it puts unit state back on the fight tab. The record
is `05-REDIRECT-D27.md`. Plans 05-12 through 05-16 close it, sequentially, and
...
| 5. Fight Loop & Playtest | 15/16 | In Progress|  |
